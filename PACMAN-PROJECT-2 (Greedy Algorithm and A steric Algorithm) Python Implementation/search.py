...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    #check if the starting node is the goal node then return it
    if problem.isGoalState(startingNode):
        return []
    #defining a visited nodes list
    visitedNodes = []
    #defining a priorityqueue
    pQueue = util.PriorityQueue()
    #((coordinate/node , action to current node , cost to current node),priority)
    #pushing the starting node in visited node list
    pQueue.push((startingNode, [], 0), 0)
    #now traverse to all nodes till queue is empty
    while not pQueue.isEmpty():
        #now pop a node from queue and get its action, previous cast
        currentNode, actions, prevCost = pQueue.pop()
        #if the node which is popped not in visited list
        if currentNode not in visitedNodes:
            #put in visited list
            visitedNodes.append(currentNode)
            #if current node is goal state return that node
            if problem.isGoalState(currentNode):
                return actions
             #now check all the successor nodes and their costs
            for nextNode, action, cost in problem.getSuccessors(currentNode):
               #Calculating path + new path
                newAction = actions + [action]
                #Calculating new cost + previous cost
                newCostToNode = prevCost + cost
                #Calculating the heuristic cost
                heuristicCost = newCostToNode + heuristic(nextNode,problem)
                #after calculating the total cost and path , push it in queue
